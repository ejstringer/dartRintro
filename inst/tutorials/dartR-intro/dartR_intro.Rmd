...
But maybe we want a complete fresh start. We can remove all data created during our R session by navigating to the *Environment* pane and clicking on the little broom icon 🧹. 
...
::: {.my-solution icon="false"}
If you haven't already, create a project using the RStudio menu (File - New Project). Your project will become the default directory for files but you can change the the directory by using The RStudio menu (Session - Set Working Directory). Alternatively use `setwd()`
:::
...
```{r, eval = FALSE}
gl <- gl.read.dart(filename="./data/Report_DTym25-13579_10_moreOrders_SNP_1.csv",
